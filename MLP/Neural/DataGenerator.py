...
class FoldGen:
    ...
    def fold_gen_pair(self, batch_size, fold_n, n=0):
        ...
        train_gen = DataGenerator88Mem(self.file_name, batch_size, 1.0 - test_ratio)
        test_gen = DataGenerator88Mem(self.file_name, batch_size, test_ratio)
...
# Should use DataGenerator88 or DataGenerator88Mem instead!!!
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.file_name + '.h5', 'r') as h5f:
            ...
            for i in range(PITCH_RANGE):
                final_label.append(label[i + PITCH_RANGE, :])

        return data, final_label

    
class DataGenerator88Mem(DataGenerator):
    def __init__(self, file_name, batch_size, used_ratio, is_test=False):
        super().__init__(file_name, batch_size, used_ratio, is_test)
    ...
    def __getitem__(self, index):
        ...
        data = self.data[:, indices_to_use].T
        label = self.label[:, indices_to_use]
...
class EpochGraphCallback(Callback):
    def __init__(self, file_name):
        ...
        with h5py.File(file_name + '.h5', 'w') as h5f:
            # epoch, loss, val_loss
            h5f.create_dataset('data', shape=(0, 4), compression='gzip', chunks=True, maxshape=(None, 4))

    def on_epoch_begin(self, epoch, logs=None):
        self.timer.tic()

    def on_epoch_end(self, epoch, logs=None):
        time_spent = self.timer.toc()
        print('Epoch #{}, loss={}, val_loss={}, time={}s'.format(epoch, logs['loss'], logs['val_loss'], time_spent))

        with h5py.File(self.file_name + '.h5', 'a') as h5f:
            h5f['data'].resize(h5f['data'].shape[0] + 1, axis=0)

            h5f['data'][-1:] = [epoch, logs['loss'], logs['val_loss'], time_spent]
