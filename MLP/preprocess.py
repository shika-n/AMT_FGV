...
def main():
    ...
    create_empty_h5('merged')
    with Timer('Batch process'):
        batch_process(file_names, 5, 'merged', duration=60)
    ...
    with Timer('Standardization'):
        ...
        means = []
        stds = []
        with h5py.File('merged.h5', 'r+') as h5f:
        ...
        with h5py.File('means_stds.h5', 'w') as h5f:
            h5f.create_dataset('means', data=means, compression='gzip')
            h5f.create_dataset('stds', data=stds, compression='gzip')
